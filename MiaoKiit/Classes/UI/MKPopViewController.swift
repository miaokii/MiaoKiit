...
//  znjtys
//
//  Created by Miaokii on 2019/8/22.
//  Copyright © 2019 联颐. All rights reserved.
//
...
open class MKPopViewController: UIViewController {
    ...
    // init方法会调用这个方法，传入参数都是nil
    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        super.init(nibName: nil, bundle: nil)
        // 当使用fullScreen时，如果控制器view有透明，则背景会显示为window，而不是其presentingViewController
        // 也就是说presentingViewController会从window栈中移除，但不会销毁
        // overFullScreen当展示控制器时，present 完成后，不会移除执行 present 操作的控制器的view和它的subViews
    ...
    // MARK: - 显示与隐藏
    func show(vc: UIViewController? = nil) {
        show(on: vc)
    }
